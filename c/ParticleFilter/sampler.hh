...
namespace smc {
  ...
  double sampler<Space>::IterateEss(void)
  {
    ...
    MoveParticles();
     ...
     for(int i = 0; i < N; i++){
				// std::cout << "The max changed"<<"Val1:"<<dMaxWeight<< "Val2" << pParticles[i].GetLogWeight() <<std::endl;
       dMaxWeight = fmax(dMaxWeight, pParticles[i].GetLogWeight());
			//  std::cout << "The one picked" << dMaxWeight<<std::endl;
		 }
     for(int i = 0; i < N; i++)
